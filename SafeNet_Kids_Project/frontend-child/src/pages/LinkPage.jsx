...
export default function LinkPage({ onLinked }) {
    ...
    const submit = async (e) => {
        ...
        try {
            const r = await axios.post('/api/child/link', { link_code: code.trim().toUpperCase(), device_id: deviceId })
            onLinked(r.data)
        } catch (ex) {
                ...
                <div className="bg-[#141d2e] border border-[#1f3050] rounded-2xl p-8 shadow-[0_4px_40px_rgba(0,0,0,.5),0_0_20px_rgba(0,230,118,.08)]">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            ...
                            <p className="text-slate-500 text-xs">Enter your Parent Linking Code</p>
                        </div>
                    </div>

                    <form onSubmit={submit}>
                        <div className="mb-4">
                            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Linking Code</label>
                    ...
                    <p className="text-center text-slate-500 text-xs mt-5">
                        Get this code from the parent's SafeNet dashboard under <strong className="text-slate-400">Settings → Children</strong>
                    </p>
                </div>
            </div>
        </div>
