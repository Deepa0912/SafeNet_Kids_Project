...
const STATUS_ITEMS = [
    { key: 'monitoring', icon: Shield, label: 'AI Monitoring', active: true, color: 'text-green-400', bg: 'bg-green-500/10 border-green-500/20' },
    { key: 'connected', icon: Wifi, label: 'Parent Connected', active: true, color: 'text-cyan-400', bg: 'bg-cyan-500/10 border-cyan-500/20' },
]

export default function StatusPage({ childInfo, onUnlink }) {
    ...
    useEffect(() => {
        ...
        const poll = async () => {
            try {
                const r = await axios.post('/api/child/heartbeat', { child_id: childInfo.child_id, is_online: true })
                setStatus(r.data)
            } catch { }
        }
        poll()
        ...
        <div className="min-h-screen flex items-center justify-center p-4"
            ...
            <div className="w-full max-w-sm">
                <div className="text-center mb-8">
                    ...
                    <p className="text-slate-400 text-sm mt-1">Hi, <strong className="text-white">{childInfo.child_name}</strong> — Device Protected</p>
                </div>
                ...
                <div className="bg-[#141d2e] border border-[#1f3050] rounded-2xl p-6 shadow-[0_4px_40px_rgba(0,0,0,.5)] space-y-3">
                    ...
                    <div className="flex items-center gap-3 bg-cyan-500/10 border border-cyan-500/20 rounded-xl px-4 py-3.5">
                        ...
                        <div>
                            ...
                            <p className="text-slate-500 text-xs">Parent dashboard is linked to this device</p>
                        </div>
                    </div>
                    ...
                    <div className="border-t border-[#1f3050] pt-3">
                        <div className="flex items-center justify-between">
                            <div className="text-xs text-slate-500">This device is safe &amp; monitored. 🛡️</div>
